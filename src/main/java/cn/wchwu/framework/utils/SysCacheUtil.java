...
public class SysCacheUtil {
    private static final String COMMON_CACHE_NAME = "cn.wchwu.dao.sys.SysMenuMapper"; // 系统用户管理、菜单管理、功能管理等都是引用的一个缓存
    private static final String DICT_CACHE_NAME = "cn.wchwu.dao.sys.SysDictMapper"; // 业务字典使用单独的缓存name
    ...
    public static Map<String, Map<String, String>> getDictMap() {
        ...
        if (value == null) {
            reloadDict();
            return (Map<String, Map<String, String>>) dictCache.getObject(DICT_MAP_CACHE_KEY);
        }
    ...
    public static Map<String, SysFunc> getAllFuncMap() {
        ...
        if (value == null) {
            reloadFuncMap();
            return (Map<String, SysFunc>) commonCache.getObject(ALL_FUNC_MAP_KEY);
        }
    ...
    public static boolean reloadDict() {
        ...
        // 查出所有字典项
        List<SysDictEntry> typeList = sysDictService.querySysDictEntry(null, null);

        // 遍历放入map缓存
        for (SysDictEntry entry : typeList) {
            if (dictMap.containsKey(entry.getDictTypeId())) {
                dictMap.get(entry.getDictTypeId()).put(entry.getDictId(), entry.getDictName());
            } else {
    ...
    public static void reloadFuncMap() {
        ...
        // 从spring容器中获取对象
        SysFuncService sysFuncService = SpringUtil.getBean(SysFuncService.class);
